...
def test_llm_service_factory(mock_key):
...
def test_heuristic_llm_strategy_incident():
    service = HeuristicLLMService()
    # Test incident format parsing
...
def test_heuristic_llm_strategy_navigation():
    service = HeuristicLLMService()
    # Test Q&A matching
    prompt_qa = "Fan Question: Is there wheelchair access? MetLife Stadium"
    res_qa = service.generate_response(prompt_qa)
    assert "Gate" in res_qa or "Transit" in res_qa


def test_heuristic_llm_strategy_sustainability():
    service = HeuristicLLMService()
    # Test eco tip matching
...
def test_heuristic_llm_strategy_default():
    service = HeuristicLLMService()
...
def test_gemini_llm_strategy_success(mock_model_class, mock_key):
    mock_key.return_value = "dummy-key"
...
def test_gemini_llm_strategy_error():
    service = GeminiLLMService("dummy-key")

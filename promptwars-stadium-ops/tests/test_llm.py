...
def test_generate_llm_response_error(mock_model_class, mock_key):
    ...
    # New design raises LLMRequestError on remote failure
    with pytest.raises(LLMRequestError) as excinfo:
        generate_llm_response("Test prompt")
    assert "Failed to fetch response from Gemini model" in str(excinfo.value)

...
def test_volunteer_roster_queries():
...
def test_volunteer_recommendation_matching():
...
def test_volunteer_recommendation_empty_roster():
    # Temporarily mark all volunteers as Unavailable/Dispatched
...
def test_analyze_and_dispatch_incident_live_llm(mock_key, mock_generate):
...
@patch('stadium_ops.core.llm.GeminiLLMService.generate_response')
@patch('stadium_ops.core.llm.get_gemini_api_key')
def test_analyze_and_dispatch_incident_live_llm_error(mock_key, mock_generate):
    mock_key.return_value = "dummy-key"
...
@patch('stadium_ops.core.llm.GeminiLLMService.generate_response')
@patch('stadium_ops.core.llm.get_gemini_api_key')
def test_sustainability_get_eco_tip_live(mock_key, mock_generate):
    mock_key.return_value = "dummy-key"
...
@patch('stadium_ops.core.llm.GeminiLLMService.generate_response')
@patch('stadium_ops.core.llm.get_gemini_api_key')
def test_sustainability_get_eco_tip_live_error(mock_key, mock_generate):
    mock_key.return_value = "dummy-key"
    mock_generate.side_effect = LLMRequestError("Gemini Server Error")
    tip = SustainabilityService.get_eco_tip(["recycle_cup"])
